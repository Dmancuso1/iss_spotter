...
const fetchISSFlyOverTimes  = function(coords, callback) {
  ...
  request(url, (error, response, body) => {
    ...
    if (response.statusCode !== 200) {
      callback(Error(`Status Code ${response.statusCode} when fetching ISS TIMES for Coords: ${body}`), null);
      return;
    }
    ...
    // for (let pass in passes['response']) {
    //   console.log(passes['response'][pass])
    //   console.log(`duration --- ${passes['response'][pass]['duration']}`)
    // }
